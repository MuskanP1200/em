...
if settings.AUTH_ENABLED:
    ...
    REDIRECT_URI = f"{settings.REDIRECT_URI}"
...
def build_template_user(user: dict) -> dict:
    ...
    """
    display_name = (user.get("name") or user.get("email") or "Dev Tester").strip()

    parts = display_name.replace(".", " ").replace("_", " ").split()
    initials = "".join(part[0] for part in parts[:2]).upper() or "DT"
...
async def index(request: Request):
    if not settings.AUTH_ENABLED:
        ...
        return templates.TemplateResponse(
            request=request,
            name="index.html",
            context={"user": build_template_user(DEV_USER)},
        )
    ...
    return templates.TemplateResponse(
        request=request,
        name="index.html",
        context={"user": build_template_user(user)},
    )
