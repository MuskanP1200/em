...
class ContextFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        record.AppName = getattr(settings, "APP_NAME", "unknown-app")

        record.AppVersion = getattr(settings, "APP_VERSION", "0.0.0")

        record.Env = str(getattr(settings, "APP_ENVIRONMENT", "dev")).lower()
...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        log_payload = {
            ...
            "AppName": getattr(record, "AppName", None),
            "AppVersion": getattr(record, "AppVersion", None),
            "Env": getattr(record, "Env", None),
            "EventName": getattr(record, "EventName", None) or record.getMessage(),
            "EventType": getattr(record, "EventType", "custom"),
            "GenericPath": getattr(record, "GenericPath", None),
            "SpanId": getattr(record, "SpanId", None),
            "TraceId": getattr(record, "TraceId", None),
            "WorkflowId": getattr(record, "WorkflowId", None),
