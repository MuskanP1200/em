...
def _derive_line_ai_status(discount_match: Optional[str]) -> Optional[str]:
    """Map cdr_line_detail discount_match → frontend ai_status for each line item."""
    if discount_match == "match":
        return "approved"
    if discount_match == "mismatch":
        return "flagged"
    return None
...
async def fetch_incident_list(
    ...
    for r in rows:
        ...
        computed_status = _derive_status(r["estimate_match"])
...
def _build_line_items(parts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i, p in enumerate(parts, start=1):
        try:
            raw_price = p.get("part_price")
            items.append(
                {
                    "line": str(i).zfill(3),
                    "op": _safe_str(p.get("op_code")) or None,
                    "description": _safe_str(p.get("line_dsc")) or None,
                    "type": _safe_str(p.get("part_type")) or None,
                    "part_num": _safe_str(p.get("part_num")) or None,
                    "price": _fmt_money(raw_price) if raw_price is not None else None,
                    "qty": _safe_str(p.get("qty")) or None,
                    "labor": _safe_str(p.get("labor_hrs")) or None,
                    "paint": _safe_str(p.get("paint_hrs")) or None,
                    "ai_status": _derive_line_ai_status(p.get("discount_match")),
                    "flag_special": False,
                }
            )
        except Exception as e:
...
def _build_breakdown(core, parts):
    ...
    by_type = {"labor": [], "parts": [], "materials": [], "misc": []}
    for row in em_rows:
    ...
    return {
        ...
        "materials": _build_simple_card(
            by_type["materials"], default_label="Paint materials"
        ),
            ...
            items.append(
                {
                    ...
                    "ai_status": _match_to_status(r.get("overall_lbr_match")),
                    "negative": False,
                }
            )
        except Exception as e:
...
def assemble_incident_detail(
    ...
    # Threshold comparison for total bar tag
    try:
        threshold = float(core.get("threshold") or 0)
    ...
    return {
        "topbar": topbar,
        "progress_tabs": _build_progress_tabs(core),
