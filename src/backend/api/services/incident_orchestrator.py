...
def _build_progress_tabs(detail: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Build the 5 progress tabs by rolling up ai_status from the already-assembled sections.

    Single source of truth: uses the exact `ai_status` strings that the UI renders,
    so a tab's color always agrees with what the user sees in its section.

    Call AFTER vehicle_info, breakdown, and total are populated in `detail`.
    """
    vi = detail.get("vehicle_info", {}) or {}
    bd = detail.get("breakdown", {}) or {}
    total = detail.get("total", {}) or {}

    labor_items = (bd.get("labor", {}) or {}).get("items", []) or []
    parts_subs = (bd.get("parts", {}) or {}).get("subsections", []) or []
    mm_items = (bd.get("materials_misc", {}) or {}).get("items", []) or []

    # Each tab below: 1 expression, says exactly what it rolls up.
    return [
        {
        ...
        },
        {
            "label": "Parts",
            "status": _tab_status(
                [s.get("subtotal_ai_status") for s in parts_subs]
                + [s.get("adjustment_ai_status") for s in parts_subs]
            ),
            "target": "section-breakdown",
        },
        {
            "label": "Labor",
            "status": _tab_status([i.get("ai_status") for i in labor_items]),
            "target": "section-breakdown",
        },
        {
            "label": "Materials & Misc",
            # EM module not built yet — explicit fallback to pending when empty
            "status": (
                _tab_status([i.get("ai_status") for i in mm_items])
                if mm_items
                else "pending"
            ),
            "target": "section-breakdown",
        ...
        {
            "label": "Total",
            "status": total.get("ai_status") or "pending",
