...
def _derive_line_ai_status(value: Any) -> Optional[str]:
    ...
    # str(True).lower() = 'true' would otherwise miss the 'match' branch.
    if isinstance(value, bool):
        return "approved" if value else "flagged"
    v = str(value).strip().lower()
    if v in ("match", "true"):
...
def _build_breakdown(core):
    ...
    materials_misc_card["findings"] = _safe_str(core.get("paint_findings"))  # ← NEW
...
def _build_parts_card(rows):
    ...
    for r in rows:
        try:
            ...
            # try:
            #     expected_adj = float(r.get("expected_adj_amt") or 0)
            # except (TypeError, ValueError):
            #     expected_adj = 0.0
            ...
            try:
                ...
                print(f"expected_net 1 = {expected_net}")
            except (TypeError, ValueError):
                expected_net = expected_gross + expected_adj_calc
                print(f"expected_net 2 = {expected_net}")
            # ── Percentages ────────────────────────────────────────
            try:
            ...
            if (
                # actual_gross == 0
                # and actual_adj == 0
                subtotal_status not in ("approved", "flagged")
            ...
            print(f"total_per_ai ={total_per_ai}")
    ...
    for r in rows:
        try:
            ...
            if is_paint:
                ...
                # # ── Build labels (same hrs, different rate) ───────
                # def _paint_label(rate):
                #     if paint_hrs and rate:
                #         return (
                #             f"{base_label} ({paint_hrs:.1f} hrs @ {_fmt_money(rate)})"
                #         )
                #     return base_label

                # actual_label = _paint_label(actual_rate)
                # expected_label = _paint_label(expected_rate)
...
def assemble_incident_detail(
    ...
    #     total_tag = (
    #         "Above threshold"
    #         if threshold and total_actual > threshold
