...
def _build_breakdown(core):
    ...
    parts_card["subtot_findings"] = _safe_str(core.get("parts_subtot_findings"))
...
def _build_parts_card(rows):
    ...
    for r in rows:
        try:
            ...
            try:    expected_adj_calc = float(r.get("expected_adj_amt_calc") or 0)
            except (TypeError, ValueError): expected_adj_calc = 0.0
            ...
            try:    expected_net = float(r.get("expected_net_amt_calc") or 0)
            except (TypeError, ValueError): expected_net = expected_gross + expected_adj_calc
            ...
            adjustment_status = _match_to_status(r.get("adj_compliance_match"))
