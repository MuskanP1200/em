...
line_items_agg AS (
        ...
        COALESCE(
            JSON_AGG(
                JSON_BUILD_OBJECT(
                    ...
                    -- Line-level labour verdict (per cieca_dtl_hdr_id), NOT the subtotal
                    -- verdict — so a subtotal issue no longer reddens every labour line.
                    'lbr_match',           CASE
                                                WHEN eld_line.line_lbr_rate_match IS TRUE  THEN 'Match'
                                                WHEN eld_line.line_lbr_rate_match IS FALSE THEN 'No Match'
                                                ELSE NULL
                                           END,
    ...
    LEFT JOIN {schema}.{em_line_detail} eld_line
           ON el.est_id::bigint::text = eld_line.est_id::bigint::text
          AND el.cieca_dtl_hdr_id::text = eld_line.cieca_dtl_hdr_id::text
...
parts_subtot_findings_agg AS (
    SELECT
        emsd.est_id,
        STRING_AGG(DISTINCT emsd.parts_subtot_mismatch_reason, ' | ') AS parts_subtot_findings
    FROM {schema}.{em_subtot_detail} emsd
    WHERE emsd.est_id::bigint::text = $1
        ...
        COALESCE(
            JSON_AGG(
                JSON_BUILD_OBJECT(
                    ...
                    'expected_gross_amt',   emsd.expected_gross_amt,    -- AI gross (line sum)
                    'actual_adj_amt',       s.adj_tot_amt,              -- actual adjustment
                    'expected_adj_amt',     emsd.expected_adj_amt,      -- AI adjustment (line sum)
                    'expected_adj_amt_calc', emsd.expected_adj_amt_calc, -- AI adjustment (gross x expected %)
                    'actual_net_amt',       s.tot_amt,                  -- actual net
                    'expected_net_amt',     emsd.expected_net_amt,      -- AI net (line sum)
                    'expected_net_amt_calc', emsd.expected_net_amt_calc, -- AI net (gross + calc discount)
                    'actual_adj_pct',       emsd.adj_pct,               -- actual %
                    'expected_adj_pct',     emsd.expected_adj_pct,      -- AI %
                    'parts_gross_match',    emsd.parts_gross_match,     -- gross consistency (line vs subtotal)
                    'adj_match',            emsd.adj_match,             -- adjustment consistency (line vs subtotal)
                    'adj_pct_match',        emsd.adj_pct_match,         -- discount % compliance (actual vs CDR)
                    'adj_compliance_match', emsd.adj_compliance_match,  -- discount $ compliance (actual vs calculated)
...
SELECT
    ...
    li.line_items_json,
    -- Parts findings (LLM line-level)
    pfa.parts_findings,
    -- Parts subtotal mismatch reasons
    psfa.parts_subtot_findings,
    -- Labor findings
    lfa.labor_findings,
...
LEFT JOIN parts_subtot_findings_agg psfa ON eb.est_id::bigint::text = psfa.est_id::bigint::text
