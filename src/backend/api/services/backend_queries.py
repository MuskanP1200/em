...
SELECT es.est_id::bigint::text as est_id,       --  TODO check if its claimid or est_id
        es.estimate_match,                       -- TODO currently its est_match shouldbe overall
        ar.vin                                   -- TODO change to make or model
FROM {schema}.{em_summary_table} es
...
line_items_agg AS (
        ...
        COALESCE(
            JSON_AGG(
                JSON_BUILD_OBJECT(
                    ...
                    'qty',                 el.dtl_part_nbr_qty,
                    'labor_hrs',           el.dtl_lbr_hr_qty,
                    'labor_rate',          NULL,
                    'paint_hrs',           NULL,
                    'labor_total',         el.dtl_lbr_tot_amt,
                    'other_charges_match', emld.other_charges_match
    ...
    LEFT JOIN {schema}.{em_line_detail} emld
           ON el.est_id::bigint::text = emld.est_id::bigint::text
          AND el.line_nbr::text       = emld.line_nbr::text
