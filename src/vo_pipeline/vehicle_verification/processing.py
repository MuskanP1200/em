...
def process_est_prefix(
    ...
    log.info(
        "[prefix=%s] Fetched %d files. Breakdown -> images=%d, thumbs=%d, "
        "non_thumbs=%d, pdfs=%d, others=%d",
        prefix,
        len(blob_names),
        len(images),
        len(thumb_set),
        len(non_thumb_images),
        len(pdfs),
        len(others),
    )
    ...
    log.info(
        "[prefix=%s] Processed images=%d; with_text=%d; without_text=%d",
        prefix,
        len(non_thumb_images),
        len(with_text),
        len(without_text),
    ...
    log.info(
        "[prefix=%s] Classified images=%d; breakdown: %s",
        prefix,
        sum(classification_counts.values()),
        cls_summary,
    )
    log.info(
        "[prefix=%s] VIN result -> matches_in_ocr=%d, matches_in_vlm=%d, "
        "best_match_vin=%s, est_vin_min_mismatches=%s, vin_status=%s",
        prefix,
        vin_ocr_hits,
        vin_vlm_hits,
        est_best_match_vin or "None",
        str(est_vin_min_mismatches) if est_vin_min_mismatches is not None else "None",
        str(vin_status),
    )
    log.info(
        "[prefix=%s] License Plate result -> matches_in_ocr=%d, matches_in_vlm=%d, "
        "best_match_plate=%s, est_plate_min_mismatches=%s, plate_status=%s",
        prefix,
        plate_ocr_hits,
        plate_vlm_hits,
        est_best_match_plate or "None",
        (
            str(est_plate_min_mismatches)
            if est_plate_min_mismatches is not None
            else "None"
        ),
        str(plate_status),
    )
    # BUG-4 FIX: log message labelled "Odometer" but logged plate_status and plate
    # variable names. Now correctly logs odometer-specific variables.
    log.info(
        "[prefix=%s] Odometer result -> matches_in_ocr=%d, matches_in_vlm=%d, "
        "best_match_odometer=%s, est_odometer_min_mismatches=%s, odometer_status=%s",
        prefix,
        odometer_ocr_hits,
        odometer_vlm_hits,
        est_best_match_odometer or "None",
        (
            str(est_odometer_min_mismatches)
            if est_odometer_min_mismatches is not None
            else "None"
        ),
        str(odometer_status),
    )
    log.info("Completed processing for prefix=%s", prefix)
