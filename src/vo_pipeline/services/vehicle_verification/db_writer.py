...
class DBWriter:
    ...
    def __init__(
        ...
        self._create_folders_sql = self._make_create_folders_sql()
        self._create_images_sql = self._make_create_images_sql()
        self._upsert_folder_sql = self._make_upsert_folder_sql()
        self._upsert_image_sql = self._make_upsert_image_sql()
    ...
    def _connect(self, pg_cfg: Dict[str, Any]) -> None:
        try:
            ...
            logger.info("DBWriter connected to Postgres")
...
"""
    ...
    def upsert_folder(self, result: Dict[str, Any]) -> bool:
        ...
        with self._lock:
            try:
                ...
                logger.info(
                    "DBWriter: upserted folder=%s (%d images)",
                    folder_name,
                    len(image_rows),
                )
                return True
    ...
    def close(self) -> None:
        ...
        if self._conn and not self._conn.closed:
            self._conn.close()
            logger.info("DBWriter: connection closed")
