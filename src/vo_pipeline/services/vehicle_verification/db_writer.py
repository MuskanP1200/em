...
_CHUNK_SIZE = (
    500  # rows per bulk INSERT — keeps param count well under psycopg2's 65 535 limit
)
...
FOLDERS_TABLE: str = f"{_FOLDERS_NAME}"
IMAGES_TABLE: str = f"{_IMAGES_NAME}"
...
_DDL_FOLDERS = f"""
CREATE TABLE IF NOT EXISTS {OUTPUT_SCHEMA}.{FOLDERS_TABLE} (
...
_DDL_IMAGES = f"""
CREATE TABLE IF NOT EXISTS {OUTPUT_SCHEMA}.{IMAGES_TABLE} (
...
INSERT INTO {OUTPUT_SCHEMA}.{FOLDERS_TABLE} (
...
def reset_vi_tables() -> None:
    ...
    with get_engine().begin() as conn:
        conn.execute(
            text(f"DROP TABLE IF EXISTS {OUTPUT_SCHEMA}.{IMAGES_TABLE} CASCADE")
        )
        conn.execute(
            text(f"DROP TABLE IF EXISTS {OUTPUT_SCHEMA}.{FOLDERS_TABLE} CASCADE")
        )
...
def _bulk_upsert_images(conn, image_rows: List[Dict[str, Any]]) -> None:
    ...
    for offset in range(0, len(image_rows), _CHUNK_SIZE):
        ...
        sql = text(  # nosec B608
            f"INSERT INTO {OUTPUT_SCHEMA}.{IMAGES_TABLE} ({col_list}) "  # nosec B608
            f"VALUES {', '.join(placeholders)} "  # nosec B608
            f"ON CONFLICT (folder_name, image_path) DO UPDATE SET {set_clause}"  # nosec B608
        )  # nosec B608
