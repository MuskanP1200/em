...
def _execute_request(
    ...
    *,
    ...
    try:
        ...
        logger.error("%s: HTTP error", label, exc_info=True)
        raise APIClientError(f"{label}: HTTP failure") from e

    except ET.ParseError as e:
        logger.error("%s: XML parse error", label, exc_info=True)
        raise APIClientError(f"{label}: invalid XML") from e

    except Exception:
        logger.error("%s: unexpected error", label, exc_info=True)
        raise
...
def search_estimates(
    ...
    while True:
        ...
        if not items:
            if start_row == 1:
                logger.warning(
                    "SearchEstimates: no result for status=%s group=%s",
                    status_code,
                    group,
                )
            break
            ...
            results.append(
                {
                    ...
                    "claim_number": _get_text(item, "ClaimNumber", _NS_EST),
                    ...
                    "est_stat_typ_id": (
                        status_el.findtext(f"{{{_NS_TYP}}}Id")
                        if status_el is not None
                        else None
                    ),
                    "est_stat_typ_cde": (
                        status_el.findtext(f"{{{_NS_TYP}}}Code")
                        if status_el is not None
                        else None
                    ),
                    "est_stat_typ_dsc": (
                        status_el.findtext(f"{{{_NS_TYP}}}Description")
                        if status_el is not None
                        else None
                    ),
                    "primary_adjuster_user_id": _get_text(
                        item, "PrimaryAdjusterUserId", _NS_EST
                    ),
                    "primary_adjuster_first_name": _get_text(
                        item, "PrimaryAdjusterFirstName", _NS_EST
                    ),
                    "primary_adjuster_last_name": _get_text(
                        item, "PrimaryAdjusterLastName", _NS_EST
                    ),
                    "est_received_dt_str": _get_text(
                        item, "EstimateReceivedDateTimeString", _NS_EST
                    ),
                    "est_received_dt": _get_text(
                        item, "EstimateReceivedDateTime", _NS_EST
                    ),
                    "managed_tow_followup_status": _get_text(
                        item, "ManagedTowFollowUpStatus", _NS_EST
                    ),
                    "manual_estimate_ind": _get_text(
                        item, "ManualEstimateInd", _NS_EST
                    ),
                    "note_to_shop": _get_text(item, "NoteToShop", _NS_EST),
                }
            )

        logger.info("SearchEstimates start_row=%d → %d items", start_row, len(items))
...
def get_estimate_detail(token: str, est_id: str) -> pd.DataFrame:
    ...
    if est_info is None:
        logger.warning("est_id %s: no EstimateInfo in detail response", est_id)
...
def get_cdr_group_vendor(token: str, vendor_id: str, group_number: str) -> dict:
    ...
    if cdr is None:
        logger.warning(
            "vendor=%s group=%s: missing CDRGroupVendor element",
            vendor_id,
            group_number,
        )
...
def get_repair_incident_detail(token: str, repair_incident_id: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(
            "RepairIncidentId %s: fetch failed, dmg_dsc will be None",
            repair_incident_id,
        )
        return None
    ...
    if el is None:
        logger.warning(
            "RepairIncidentId %s: no RepairIncidentSearchResultItem in detail response",
            repair_incident_id,
        )
        return None
...
def get_image_bytes(token: str, attachment: dict) -> tuple[str, bytes]:
    ...
    if not image_b64:
        raise APIClientError(
            f"GetAttachmentBytes({attachment_id}): no image bytes in response"
        )
