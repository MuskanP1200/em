...
def fetch_estimate_details(
    ...
    """Fetch electronic estimate XML, CDR detail for each est_id. Returns (est_line_df, subtot_df)."""
    ...
    log.info(
        "Loading %d estimates from API (max_workers=%d)",
        len(est_ids),
        max_workers,
    )
...
def save_estimate_details(
    est_line_df: pd.DataFrame,
    subtot_df: pd.DataFrame,
    table_names: tuple[str, str],
    schema: str = "ice",
) -> None:
    """Write est_line and subtot DataFrames to Postgres. Call after any enrichment (e.g. CVD)."""
