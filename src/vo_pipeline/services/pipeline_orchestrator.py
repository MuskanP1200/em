...
from sql_connection import get_engine, read_table, read_query, build_in_clause, write_table
...
EM_VI_WORKERS = 1  # number of estimates processed concurrently in Steps 2+3
...
def _get_incomplete_est_ids(run_vi: bool, run_em: bool) -> list[str]:
    ...
    try:
        df = read_query(f"SELECT est_id FROM {PIPELINE_STATUS_TABLE} WHERE {where}")  # nosec B608
...
def _update_pipeline_status(est_id: str, stage: str, success: bool) -> None:
    ...
    try:
        with get_engine().begin() as conn:
            conn.execute(
                text(  # nosec B608
                    f"INSERT INTO {PIPELINE_STATUS_TABLE} (est_id, {col}_completed_at, {col}_success, {col}_attempt_count) "
                    f"VALUES (:est_id, NOW(), :success, 1) "
                    f"ON CONFLICT (est_id) DO UPDATE SET "
                    f"    {col}_completed_at  = NOW(), "
                    f"    {col}_success       = :success, "
                    f"    {col}_attempt_count = {PIPELINE_STATUS_TABLE}.{col}_attempt_count + 1, "
                    f"    update_timestamp    = NOW()"
                ),
                {"success": success, "est_id": est_id},
            )
    except Exception as e:
        logger.warning("Could not update pipeline status for est_id=%s stage=%s: %s", est_id, stage, e)
...
def run_pipeline():
    ...
    if incomplete:
        logger.info("Found %d incomplete est_id(s) from previous runs — adding to batch.", len(incomplete))
    est_ids = list(dict.fromkeys(est_ids + incomplete))  # union, deduplicated, order preserved
    ...
    if RUN_VI and RUN_EM:
        ...
        try:
            ...
            with get_engine().begin() as conn:
                conn.execute(
                    text(f"DELETE FROM {OUTPUT_SCHEMA}.{TABLE_OVERALL_SUMMARY} WHERE est_id IN ({in_clause})"),  # nosec B608
                )
            write_table(
                merged,
                TABLE_OVERALL_SUMMARY,
