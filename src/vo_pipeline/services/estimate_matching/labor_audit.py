...
def _validate_labour_type(
    ...
    # ── Amounts — store ACTUAL (unrounded); match on $1 tolerance ──────────────
    expected_lbr_amt = (
        round(expected_rate * expected_hrs, 2) if expected_rate is not None else None
    )
    lbr_amt_match = (
...
def add_line_labour_rate_match(est_line_df: pd.DataFrame) -> pd.DataFrame:
    ...
    has_data = (
        df["dtl_lbr_hr_qty"].notna()
        & df["dtl_lbr_hr_qty"].gt(0)
    ...
    can_compare = (
        df["actual_line_lbr_rate"].notna() & df["expected_line_lbr_rate"].notna()
    )
    match = df["actual_line_lbr_rate"].round(ROUND_DECIMALS) == df[
        "expected_line_lbr_rate"
    ].round(ROUND_DECIMALS)
