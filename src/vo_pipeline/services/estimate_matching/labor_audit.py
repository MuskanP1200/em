...
from estimate_matching.config import LABOR_TYPE_RATE_MAP, RATE_COLS, ROUND_DECIMALS
...
def _validate_labour_type(
    ...
    hrs_match = round(actual_hrs, 1) == round(expected_hrs, 1)
    ...
    if overall_lbr_match == "No Match":
        ...
        lbr_mismatch_reason = f"{label}:: {','.join(reasons)}"
    else:
...
def match_labor_subtotals(
    ...
    # Aggregate line-level data and merge subtotal rates
    grouped = (
