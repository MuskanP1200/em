...
from estimate_matching.db_writer import (
    save_results,
    LINE_DETAIL_COLS,
    SUBTOT_DETAIL_COLS,
)  # noqa: E402
...
from estimate_matching.config import ROUND_DECIMALS  # noqa: E402
...
def _build_output_tables(
    ...
    line_detail = line_detail.reindex(
        columns=[c for c in LINE_DETAIL_COLS if c in line_detail.columns]
    )

    # ── subtot_detail ─────────────────────────────────────────────────────────
    df_parts_sub = (
        df_parts_subtot_audit.copy()
    ...
    df_lbr_sub = (
        df_lbr_subtot_audit.copy() if not df_lbr_subtot_audit.empty else pd.DataFrame()
    )
    if not df_lbr_sub.empty:
        df_lbr_sub.rename(
            columns={"cieca_lbr_typ_dsc": "cieca_tot_typ_dsc"}, inplace=True
        )
        df_lbr_sub["subtot_type"] = "labor"

    df_paint_sub = (
        df_paint_subtot_audit.copy()
    ...
    # Keep only columns defined in the DDL — DDL is the single source of truth
    subtot_detail = subtot_detail.reindex(
        columns=[c for c in SUBTOT_DETAIL_COLS if c in subtot_detail.columns]
    )
