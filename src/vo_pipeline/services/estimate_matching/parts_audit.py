...
def match_parts_subtotals(
    ...
    PARTS_MAP = {
        ...
        "Parts - Remanufactured": "Parts - Rechromed",
        "Parts - Recycled": "Parts - Like Kind and Quality (LKQ)",
    }
    _RATE_COL_MAP = {
    ...
    grouped["expected_net_amt"] = (
        grouped["expected_gross_amt"] + grouped["expected_adj_amt"]
    ).round(2)
    ...
    net_match = merged["expected_net_amt"].round(ROUND_DECIMALS) == merged[
        "tot_amt"
    ].round(ROUND_DECIMALS)
    ...
    merged["adj_compliance_match"] = np.where(
        ...
        "No Match",
        np.where(
            no_expected, "No Match", np.where(adj_compliance, "Match", "No Match")
        ),
    )
    ...
    merged["parts_subtot_mismatch_reason"] = merged.apply(
        _parts_subtot_mismatch_reason, axis=1
    )
