...
def match_parts_subtotals(
    ...
    def _parts_subtot_mismatch_reason(row) -> str | None:
        ...
        if row["parts_gross_match"] == "No Match":
            reasons.append(
                f"Gross mismatch (line: ${row['line_tot_part_amt']:.2f}, subtot: ${row['gross_amt']:.2f})"
            )
        if row["adj_match"] == "No Match":
            reasons.append(
                f"Adjustment mismatch (line: ${row['line_adj_amt']:.2f}, subtot: ${row['adj_tot_amt']:.2f})"
            )
        if row["parts_net_match"] == "No Match":
            reasons.append(
                f"Net mismatch (line: ${row['line_net_amt']:.2f}, subtot: ${row['tot_amt']:.2f})"
            )
        if row["adj_pct_match"] == "No Match":
            reasons.append(
                f"Discount % mismatch (expected: {row['expected_adj_pct']}%, actual: {row['adj_pct']}%)"
            )
